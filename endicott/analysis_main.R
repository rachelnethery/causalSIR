...
for (M in c(3,5)){
  for (metho in c("mahalanobis","logit","GAMlogit")){
    ...
    write.csv(temptab,file=paste0('balance_',M,'_',metho,'.csv'))
    ...
    for (ctype in c("Kidney","Bladder")){
      ...
      write.csv(predsum, file=paste0(ctype,'_predsum_',M,'_',metho,'.csv'),row.names=F)
      ...
      ## output traceplots ##
      pdf(paste0(ctype,'_traceplots_',M,'_',metho,'.pdf'),width=12)
      par(mfrow=c(2,6))
      ...
      if (M==3 & metho=="mahalanobis"){
        sink(paste0(ctype,'_results.txt'))
        cat('\n')
        cat("=============================\n")
        ...
        sink(paste0(ctype,'_results.txt'),append=T)
        cat('\n')
        cat("=============================\n")
        cat('M=',M,', Method=',metho,'\n')
        cat("=============================\n")
