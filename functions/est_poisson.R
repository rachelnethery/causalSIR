est_poisson<-function(X,Y_all,offs,nsamp=ncol(as.matrix(Y_all)),pburn=.25,tau=.001){
  ...
  if (ncol(as.matrix(Y_all))==1){
    C<-vcov(glm(Y_all~X,family='poisson'))
  } else {
    C<-vcov(glm(round(apply(Y_all,1,mean))~X,family='poisson'))
  }
  Cinv<-solve(C)
