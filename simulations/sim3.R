#################################################################################################
## CSIR ESTIMATION SIMULATIONS WITH NO X->T RELATIONSHIP, X->Y RELATIONSHIP, T->Y RELATIONSHIP ##
## created by Rachel Nethery                                                                   ##
#################################################################################################

process<-as.numeric(as.character(commandArgs(trailingOnly=TRUE)))

trueSIR<-1.1+(process*.1)
...
betaXEY<-matrix(c(-5,.007,.015,.03,-.001,-.02,.004,.002,-.005,log(trueSIR)),ncol=1)
loglambda<-as.matrix(cbind(1,Xan,E))%*%betaXEY
...
save(csir_results,pr_results,cdc_results,file=paste0('sim3_',trueSIR,'.RData'))
